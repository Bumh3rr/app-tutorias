...
public class TutoradoController {
    ...
    public String obtenerVistaListaTutorados(
        ...
        try {
            switch (tipoBusqueda) {
                ...
                default -> {
                    paginaTutorados = this.tutoradoService.obtenerPorEstadoPaginado(filtroEstado, page, pageSize, sortBy, sort);
                    model.addAttribute("filtro", null);
                }
            }
        } catch (Exception e) {
    ...
    public String guardarTutorado(
        ...
        try {
            ...
            attributes.addFlashAttribute("credenciales_nombre", tutorado.getNombre() + " " + tutorado.getApellido());
            attributes.addFlashAttribute("credenciales_rol", "Tutorado");
            attributes.addFlashAttribute("credenciales_correo", tutorado.getEmail());
            attributes.addFlashAttribute("credenciales_password", tutorado.getNumeroControl());
            ...
            attributes.addFlashAttribute("msg_warning", "Ya existe un " + e.getTipoEntidad() + " inactivo con el mismo " +
                e.getCampoConflicto() + ": \"" + e.getValorConflicto() + "\". <a href='/admin/tutorado?filtroEstado=inactivos'>Ver inactivos →</a>");
    ...
    public String eliminarTutorado(@PathVariable Integer id, RedirectAttributes attributes) {
        try {
            ...
            attributes.addFlashAttribute("msg_success", "Tutorado dado de baja. El acceso a la plataforma ha sido bloqueado.");
        } catch (Exception e) {
    ...
    public String reactivarTutorado(@PathVariable Integer id, RedirectAttributes attributes) {
        try {
            this.tutoradoService.reactivar(id);
            attributes.addFlashAttribute("msg_success", "Tutorado reactivado. El acceso a la plataforma ha sido restaurado.");
        } catch (Exception e) {
            attributes.addFlashAttribute("msg_error", "Error al reactivar: " + e.getMessage());
        }
        return "redirect:/admin/tutorado?filtroEstado=activos";
    }
