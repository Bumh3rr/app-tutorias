    ...
    public String obtenerVistaListaDetecciones(
        ...
        try {
            if ("tutorado".equals(tipoBusqueda) && idTutorado != null) {
                ...
                try {
                    Date ini = new SimpleDateFormat("yyyy-MM-dd").parse(fechaInicio);
                    Date fin2 = new Date(new SimpleDateFormat("yyyy-MM-dd").parse(fechaFin).getTime() + 86399999L);
                    java.util.List<com.bumh3r.entity.DeteccionNecesidades> lista = this.deteccionNecesidadesService.buscarPorFechaRegistro(ini, fin2);
                    pageResult = new org.springframework.data.domain.PageImpl<>(paginate(lista, pageable), pageable, lista.size());
                } catch (Exception ex) {
